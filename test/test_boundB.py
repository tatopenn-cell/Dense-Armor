...
def calcola_chunk_ottimale():
    ...
    # Singolo frame: 32x32 pixel * 8 byte (float64) * 5.0 (overhead buffer XLA/NumPy)
...
def test_geometric_spatial_attacks():
    """Rotazione/traslazione affine e deformazione elastica: 50000 passi ciascuna
    contro AdaptiveSignalStabilizer."""
    key = jax.random.PRNGKey(88)
    t_start = time.time()

    print(f"\n{STEPS_LONG} passi per asse (rotazione/traslazione affine, deformazione elastica)")
    ...
    while start_idx < STEPS_LONG:
        ...
        print(f"  [{start_idx:05d}:{end_idx:05d}] blocco={current_chunk_len} RAM libera={ram_m_libera:.1f} MB")
    ...
    print("\nErrore campionato (V = MSE rispetto al riferimento):")
    checkpoints = [0, 10000, 20000, 30000, 40000, STEPS_LONG - 1]
    for check in checkpoints:
        print(f"  passo {check:05d} | V_rot: {float(v_rot[check]):.5f} | V_elastico: {float(v_elastic[check]):.5f}")

    print(f"\nTempo di calcolo totale: {t_elapsed:.2f} s")
    print(f"V_max rotazione: {float(jnp.max(v_rot)):.4f} | V_finale rotazione: {float(v_rot[-1]):.4f}")
    print(f"V_max elastico:  {float(jnp.max(v_elastic)):.4f} | V_finale elastico:  {float(v_elastic[-1]):.4f}")

    # NOTA: era presente qui un `assert t_elapsed >= 5.0` -- non verificava
    # la correttezza della difesa, solo che il calcolo fosse durato
    # "abbastanza" su QUESTA macchina. Fragile per costruzione: fallisce
    # ogni volta che l'hardware e' piu' veloce (CI verificato: fallito su
    # ubuntu/windows-latest, ogni versione Python), non un segnale di
    # regressione reale. Rimosso -- le assert sotto (nessuna divergenza
    # numerica) sono la verifica che conta davvero.
    assert jnp.max(v_rot) < 1.20, "Divergenza numerica rilevata sull'asse rotazione"
    ...
    assert jnp.max(v_elastic) < 1.20, "Divergenza numerica rilevata sull'asse elastico"

if __name__ == "__main__":
    test_geometric_spatial_attacks()
