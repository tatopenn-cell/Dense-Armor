...
real torque-level quantities (M(q), not a velocity command).

Two later additions, both promoted from further Discovery experiments:
a `.xacro` path is expanded via the real `xacro` package (macros, `${...}`
math, `xacro:if`/`unless`, `xacro:include`) before parsing, the same
expander the ROS ecosystem itself uses; and a joint's `<mimic>` tag (e.g. a
gripper's two fingers tied together) is respected -- the mimic joint gets no
independent coordinate of its own, its motion computed from its master's via
`multiplier`/`offset`, chain-ruled onto the master's own column in the
hand-built geometric Jacobian used for the mass matrix.
"""
...
def _parse_urdf(path):
    """Parse a URDF file, or expand a `.xacro` one first via the real `xacro` package."""
