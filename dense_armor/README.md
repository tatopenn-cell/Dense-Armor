...
**Limite noto, non ancora risolto**: `fact` dipende da `log10(|valore_pulito|)` — quando il riferimento pulito attraversa lo zero (es. un'onda sinusoidale centrata in 0), i valori vicini allo zero producono un `fact` enorme, e la compressione può esplodere numericamente peggiorando il risultato rispetto alla modalità cieca. Riprodotto anche su commit precedenti a questa scoperta (non introdotto dall'integrazione memoria/resonance di Orca). Vedi `$ limiti --known` nel README principale.
